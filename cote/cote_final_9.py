...
def calculate_recursive(priority, n, expression):
    ...
    if priority[n] == '*':  # 먼저 쪼갠 애들부터 계산한다.
        res = eval('*'.join([calculate_recursive(priority, n + 1, e) for e in expression.split('*')]))
    if priority[n] == '+':
        res = eval('+'.join([calculate_recursive(priority, n + 1, e) for e in expression.split('+')]))
    if priority[n] == '-':
        res = eval('-'.join([calculate_recursive(priority, n + 1, e) for e in expression.split('-')]))
...
def solution(expression):
    ...
    for priority in priorities:
        result = int(calculate_recursive(priority, 0, expression))
